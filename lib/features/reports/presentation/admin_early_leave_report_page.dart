...
class AdminEarlyLeaveReportPage extends ConsumerStatefulWidget {
  const AdminEarlyLeaveReportPage({super.key});

  @override
  ConsumerState<AdminEarlyLeaveReportPage> createState() =>
      _AdminEarlyLeaveReportPageState();
}

class _AdminEarlyLeaveReportPageState
    extends ConsumerState<AdminEarlyLeaveReportPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final rows = <AdminEarlyLeaveReportPdfRow>[];
    for (final item in state.todayAdminAttendance) {
      final record = item['record'] as Map<String, dynamic>? ?? {};
      final approved = _toInt(record['approved_early_leave_minutes']);
      final unapproved = _toInt(record['unapproved_early_leave_minutes']);
      if (approved == 0 && unapproved == 0) continue;

      rows.add(
        AdminEarlyLeaveReportPdfRow(
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('تقرير الخروج المبكر الشهري'),
        actions: [
          IconButton(
            tooltip: 'PDF',
              ...
              await Navigator.of(context).push(
                MaterialPageRoute(
                  builder: (_) => PdfPreviewPage(
                    bytes: bytes,
                    title: 'معاينة تقرير الخروج المبكر',
                    fileName:
                    'nabd_alamal_early_leave_${_selectedMonth.year}_${_selectedMonth.month.toString().padLeft(2, '0')}.pdf',
                  ),
                ),
              );
            },
          ...
          if (rows.isEmpty)
            const Center(
              child: Padding(
                ...
                child: Text(
                  'لا توجد حالات خروج مبكر في هذا الشهر',
                  style: TextStyle(
